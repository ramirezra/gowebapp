...
func init() {
	...
	DB, err = sqlx.Open(Driver, Driver+"://"+Account+":"+Pword+"@"+Datastore+"?sslmode=disable")
	if err != nil {
		panic(err)
	}
...
func (comment *Comment) Create() (err error) {
	...
	err = DB.QueryRowx("INSERT INTO comments (content, author, post_id) VALUES($1,$2,$3) RETURNING ID", comment.Content, comment.Author, comment.Post.Id).Scan(&comment.Id)
	return
}
...
func GetPost(id int) (post Post, err error) {
	...
	err = DB.QueryRowx("SELECT id, content, author FROM posts where id=$1", id).Scan(&post.Id, &post.Content, &post.Author)
